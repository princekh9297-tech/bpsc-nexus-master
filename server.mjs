...
const ANALYTICS_ENABLED = Boolean(POSTHOG_API_KEY);
...
Always refer to answer choices using their visible letters A, B, C or D (and option text when useful). Never expose internal zero-based answer indexes such as 0, 1, 2 or 3 as the correct answer.
...
For NIVA 2.0, adapt to the requested mode: Tutor, Doubt Solver, Quiz Master, Revision Coach or Strategy.
Use the supplied vault question/context as the primary source when one is present.
Never invent a source answer, explanation, question metadata or performance statistic.
When a learner is answering a live question, do not reveal the answer unless the learner asks for it or the app context indicates the solution is being reviewed.
When discussing a source explanation, clearly distinguish "Source explanation" from "NIVA analysis".
Keep responses useful on mobile: short paragraphs, occasional simple bullets and compact exam takeaways.
Output clean plain text only. Do not use Markdown headings (#/##/###), Markdown tables, code fences, JSON wrappers, or decorative syntax. Use a simple label followed by a colon when a section label helps clarity.
If the supplied context contains numeric answer indexes, convert them to the visible option letters A, B, C or D before responding. Never tell the learner that the correct answer is 0, 1, 2 or 3.
Answer the student's actual request first and avoid generic introductory commentary.`;
...
async function sendAnalyticsToPostHog(event) {
  if (!ANALYTICS_ENABLED) return;
  const distinctId = String(event?.visitorId || "anonymous").slice(0, 128);
  const body = {
    api_key: POSTHOG_API_KEY,
    event: String(event?.event || "unknown").slice(0, 120),
    distinct_id: distinctId,
    properties: {
      ...(event?.data && typeof event.data === "object" ? event.data : {}),
      page: String(event?.page || "").slice(0, 300),
      app: "BPSC Nexus"
    },
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url === "/health") {
    return send(res, 200, { ok: true, model: MODEL, geminiKeyConfigured: !!GEMINI_API_KEY, analyticsEnabled: ANALYTICS_ENABLED });
  }
  ...
  if (req.method === "POST" && req.url === "/api/analytics") {
    ...
    req.on("end", async () => {
      try {
        ...
        const event = {
          event: String(payload.event || "unknown").slice(0, 120),
          visitorId: String(payload.visitorId || "anonymous").slice(0, 128),
          ts: payload.ts || new Date().toISOString(),
          page: String(payload.page || "").slice(0, 300),
          data: payload.data && typeof payload.data === "object" ? payload.data : {}
        };
        await sendAnalyticsToPostHog(event);
        return send(res, 204, "");
      } catch (err) {
        return send(res, 400, {error:"Invalid analytics event."});
      }
    });
    return;
  }
